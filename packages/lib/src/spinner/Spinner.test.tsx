...
describe("Spinner component tests", () => {
  test("Spinner renders with correct label", () => {
    const { getByText } = render(<DxcSpinner label="test-loading" />);
    expect(getByText("test-loading")).toBeTruthy();
  });
  test("Spinner shows value correctly", () => {
    const { getByText } = render(<DxcSpinner label="test-loading" value={75} showValue />);
    expect(getByText("75%")).toBeTruthy();
  });
  test("Small spinner hides value and label correctly", () => {
    const { queryByText, getByRole } = render(<DxcSpinner mode="small" label="test-loading" value={75} showValue />);
  ...
  test("Overlay spinner shows value and label correctly", () => {
    const { getByText } = render(<DxcSpinner mode="overlay" label="test-loading" value={75} showValue />);
    expect(getByText("test-loading")).toBeTruthy();
    expect(getByText("75%")).toBeTruthy();
  });
