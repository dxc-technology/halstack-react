...
describe("Paginator component tests", () => {
  ...
  test("Last and next buttons are disable in last page", () => {
    ...
    const { getAllByRole } = render(
      <DxcPaginator prevFunction={onClick} firstFunction={onClick} nextFunction={onClick} lastFunction={onClick} currentPage={2} itemsPerPage={10} totalItems={20}></DxcPaginator>
    );
  ...
  test("First and previous buttons are disable in first page", () => {
    const onClick = jest.fn();
    const { getAllByRole } = render(
      <DxcPaginator prevFunction={onClick} firstFunction={onClick} nextFunction={onClick} lastFunction={onClick} currentPage={1} itemsPerPage={10} totalItems={20}></DxcPaginator>
    );
    const firstButton = getAllByRole("button")[0];
    const prevButton = getAllByRole("button")[1];
    const nextButton = getAllByRole("button")[2];
