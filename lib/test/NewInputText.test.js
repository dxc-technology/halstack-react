  ...
  test("Input Non Strict - Length constraint", () => {
    const onChange = jest.fn((info) => {
      ...
      expect(info.error).toBe(
        "Please lengthen this text to 5 characters or more (you are currently using 1 characters)."
      );
    });
...
describe("NewInputText component synchronous autosuggest tests", () => {
  ...
  test("Autosuggest Strict - Length constraint", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith({
      value: "Chad",
      error: "Please lengthen this text to 5 characters or more (you are currently using 4 characters).",
    });
    fireEvent.blur(input);
    expect(
      queryByText("Please lengthen this text to 5 characters or more (you are currently using 4 characters).")
    ).toBeTruthy();
  });
