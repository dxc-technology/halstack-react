...
describe("Toggle group component tests", () => {
  ...
  test("Function on change is not called when disable", () => {
    ...
    const { getByText } = render(
      <DxcToggleGroup label="Toggle group" options={options} onChange={onChange} disabled />
    );
  ...
  test("Calls correct function on change with value when is multiple", () => {
    const onChange = jest.fn();
    const { getByText } = render(
      <DxcToggleGroup label="Toggle group" options={options} onChange={onChange} multiple />
    );
  ...
  test("Controlled multiple toggle returns always same values", () => {
    const onChange = jest.fn();
    const { getByText } = render(
      <DxcToggleGroup label="Toggle group" options={options} onChange={onChange} value={[1]} multiple />
    );
