...
describe("Password component tests", () => {
  ...
  test("onChange function is called correctly", () => {
    ...
    const { getByTestId } = render(<DxcPassword label="Password" onChange={onChange} />);
    const password = getByTestId("input-test-id");
    userEvent.type(password, "Pa$$w0rd");
    expect(onChange).toHaveBeenCalledWith({ value: "P", error: undefined });
    expect(password.value).toBe("Pa$$w0rd");
  });

  test("Clear password value", () => {
    const onChange = jest.fn();
    const { getAllByRole, getByTestId } = render(<DxcPassword label="Password" onChange={onChange} clearable />);
    const password = getByTestId("input-test-id");
    userEvent.type(password, "Pa$$w0rd");
    expect(password.value).toBe("Pa$$w0rd");
  ...
  test("Non clearable password has no clear icon", () => {
    const onChange = jest.fn();
    const { getAllByRole, getByTestId } = render(<DxcPassword label="Password" onChange={onChange} />);
    const password = getByTestId("input-test-id");
    userEvent.type(password, "Pa$$w0rd");
    expect(password.value).toBe("Pa$$w0rd");
  ...
  test("Show/hide password button works correctly", () => {
    const onChange = jest.fn();
    const { getAllByRole, getByTestId } = render(<DxcPassword label="Password" clearable onChange={onChange} />);
    const password = getByTestId("input-test-id");
    userEvent.type(password, "Pa$$w0rd");
    expect(password.value).toBe("Pa$$w0rd");
