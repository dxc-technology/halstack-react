...
describe("Accordion component tests", () => {
  ...
  test("Disabled accordion in accordion group", () => {
    const onActiveChange = jest.fn();
    const { getByText } = render(
      <DxcAccordionGroup
        margin="large"
        onActiveChange={onActiveChange}
      >
        <DxcAccordionGroup.Accordion label="Accordion1" padding="medium" margin="large" disabled={true}>
          <div>
            Text1
          </div>
        </DxcAccordionGroup.Accordion>
