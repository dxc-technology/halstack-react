...
const rows = [
  [
    ...
    {
      displayValue: <DxcButton mode="flat" iconSrc={deleteIcon} onClick={() => console.log("Clicked")} />,
    },
  ],
  [
    {
...
describe("ResultsetTable component tests", () => {
  ...
  test("Resultsettable filtering by name", () => {
  ...
  test("Resultsettable filtering with no results", () => {
    const { getByRole, queryByText } = render(
      <DxcResultsetTable columns={columns} rows={rows} itemsPerPage={3}></DxcResultsetTable>
    );
    const input = getByRole("textbox");
