...
describe("Chip component tests", () => {
  ...
  test("Calls correct function when clicking on prefix icon", () => {
    ...
    const { getByText, getByRole } = render(<DxcChip label="Chip" prefixIcon={icon} onClickPrefix={onClick} />);
    expect(getByText("Chip")).toBeTruthy();
  ...
  test("Calls correct function when clicking on suffix icon", () => {
    const onClick = jest.fn();
    const { getByText, getByRole } = render(<DxcChip label="Chip" suffixIcon={icon} onClickSuffix={onClick} />);
    expect(getByText("Chip")).toBeTruthy();
    fireEvent.click(getByRole("img"));
    expect(onClick).toHaveBeenCalled();
  });
