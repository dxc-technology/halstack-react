...
describe("TextInput component tests", () => {
  ...
  test("onChange function is called correctly", async () => {
    const onChange = jest.fn();
    const { getByRole, user } = setup(<DxcTextInput label="Input label" onChange={onChange} />);
    const input = getByRole("textbox");
  ...
  test("Clear action's tooltip is correct", async () => {
  ...
  test("Clear action onClick cleans the input", async () => {
    const { getByRole, user } = setup(<DxcTextInput label="Input label" clearable />);
    const input = getByRole("textbox");
  ...
  test("Disabled input renders with correct aria and can not be modified", async () => {
    const onChange = jest.fn();
    const { getByRole, user } = setup(<DxcTextInput label="Input label" onChange={onChange} disabled />);
    const input = getByRole("textbox");
    expect(input.getAttribute("aria-disabled")).toBe("true");
    await user.type(input, "Test");
    expect(onChange).not.toHaveBeenCalled();
  });
  test("Disabled input (action must be shown but not clickable)", async () => {
    ...
    const { getByRole, user } = setup(<DxcTextInput label="Disabled input label" action={action} disabled />);
    const input = getByRole("textbox");
  ...
  test("Action prop: image displayed with title and onClick event", async () => {
    ...
    const { getByRole, getByTestId, getByTitle, user } = setup(<DxcTextInput label="Input label" action={action} />);
  ...
  test("Input has correct accesibility attributes", async () => {
    ...
    const { getByRole, getAllByRole, user } = setup(<DxcTextInput label="Example label" clearable action={action} />);
    const input = getByRole("textbox");
...
describe("TextInput component synchronous autosuggest tests", () => {
  ...
  test("Autosuggest is displayed while the user is writing (if closed previously, if it is open stays open)", async () => {
    const onChange = jest.fn();
    const { getByRole, queryByRole, getByText, getAllByText, user } = setup(
      <DxcTextInput label="Autocomplete Countries" suggestions={countries} onChange={onChange} />
    );
    const input = getByRole("combobox");
    fireEvent.focus(input);
  ...
  test("Autosuggest closes the listbox when there are no matches for the user's input", async () => {
    const onChange = jest.fn();
    const { getByRole, queryByRole, getByText, user } = setup(
      <DxcTextInput label="Autocomplete Countries" suggestions={countries} onChange={onChange} />
    );
    const input = getByRole("combobox");
    fireEvent.focus(input);
  ...
  test("Autosuggest controlled - Suggestion selected by click", async () => {
    ...
    waitForElementToBeRemoved(queryByRole("listbox"));
  });
...
describe("TextInput component asynchronous autosuggest tests", () => {
  ...
  test("Autosuggest Esc key works while 'Searching...' message is shown", async () => {
    ...
    const onChange = jest.fn();
    const { getByRole, queryByText, queryByRole, user } = setup(
      <DxcTextInput label="Autosuggest Countries" suggestions={callbackFunc} onChange={onChange} />
    );
    const input = getByRole("combobox");
    fireEvent.focus(input);
  ...
  test("Autosuggest Esc + arrow down working while 'Searching...' message is shown", async () => {
    ...
    const onChange = jest.fn();
    const { getByRole, getByText, queryByText, queryByRole, user } = setup(
      <DxcTextInput label="Autosuggest Countries" suggestions={callbackFunc} onChange={onChange} />
    );
    const input = getByRole("combobox");
    fireEvent.focus(input);
  ...
  test("Asynchronous autosuggest closes the listbox after finishing no matches search", async () => {
    ...
    const onChange = jest.fn();
    const { getByText, getByRole, queryByRole, user } = setup(
      <DxcTextInput label="Autosuggest Countries" onChange={onChange} suggestions={callbackFunc} />
    );
    const input = getByRole("combobox");
    fireEvent.focus(input);
    await user.type(input, "Example text");
    await waitForElementToBeRemoved(() => getByText("Searching..."));
    expect(queryByRole("listbox")).toBeFalsy();
  });
