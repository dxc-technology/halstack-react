...
describe("Radio Group component tests", () => {
  ...
  test("aria-orientation attribute changes depending on stacking prop value", () => {
    const { getByRole } = render(
      <DxcRadioGroup label="test-radioGroup-label" options={options} stacking="row" />
    );
    const radioGroup = getByRole("radiogroup");
  ...
  test("The 'arrowDown' and 'arrowRight' keys move the selection to the next radio", () => {
    const onChange = jest.fn();
    const onBlur = jest.fn();
    const { getByRole, getAllByRole, container } = render(
      <DxcRadioGroup
  ...
  test("The 'arrowUp' and 'arrowLeft' keys move the selection to the previous radio", () => {
    const onChange = jest.fn();
    const onBlur = jest.fn();
    const { getByRole, getAllByRole, container } = render(
      <DxcRadioGroup
