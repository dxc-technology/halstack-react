...
import { componentTokens, advancedComponentTokens } from "./common/variables.js";
...
const parseAdvancedTheme = (advancedTheme) => {
  const allTokensCopy = JSON.parse(JSON.stringify(advancedComponentTokens));
...
const parseTheme = (theme, advancedTheme) => {
  ...
  dropdownTokens.hoverBackgroundColor =
    setOpacity(theme?.dropdown?.baseColor, 0.8) ?? dropdownTokens.hoverBackgroundColor;
  dropdownTokens.hoverBackgroundOption =
    setOpacity(theme?.dropdown?.baseColor, 0.34) ?? dropdownTokens.hoverBackgroundOption;
  ...
  headerTokens.hoverHamburguerColor =
    setOpacity(theme?.header?.hamburguerColor, 0.16) ?? headerTokens.hoverHamburguerColor;
  ...
  progressBarTokens.totalLine = setOpacity(theme?.progressBar?.baseColor, 0.34) ?? progressBarTokens.totalLine;
  ...
  sliderTokens.totalLine = setOpacity(theme?.slider?.baseColor, 0.34) ?? sliderTokens.totalLine;
  ...
  sliderTokens.disabledTrackLine = setOpacity(theme?.slider?.baseColor, 0.34) ?? sliderTokens.disabledTrackLine;
...
const ThemeProvider = ({ theme, advancedTheme, children }) => {
  const parsedTheme = useMemo(() => (theme ? parseTheme(theme) : parseAdvancedTheme(advancedTheme)), [theme]);
  return (
    <Halstack>
