  ...
  test("Uncontrolled accordion group calls correct function on click", () => {
    ...
    const { getByText, getAllByRole } = render(
      <DxcAccordionGroup margin="large" onActiveChange={onActiveChange}>
        <DxcAccordionGroup.Accordion label="Accordion1" margin="large">
          <div>
    ...
    expect(getAllByRole("button")[0].getAttribute("aria-expanded")).toBe(
      "false"
    );
    expect(getAllByRole("button")[1].getAttribute("aria-expanded")).toBe(
      "false"
    );
