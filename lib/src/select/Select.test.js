...
describe("Select component tests", () => {
  ...
  test("Non-Grouped Options - Renders SVG icons correctly when passed with options", async () => {
    const { getByRole, getAllByRole } = render(<DxcSelect label="test-select-label" options={svgIconOptions} />);
    const select = getByRole("combobox");
    await userEvent.click(select);
  ...
  test("Non-Grouped Options - Renders string url icons correctly when passed with options", async () => {
    const { getByRole, getAllByRole } = render(
      <DxcSelect label="test-select-label" options={urlIconOptions} optional />
    );
    const select = getByRole("combobox");
    await userEvent.click(select);
  ...
  test("Grouped Options - Renders icons correctly when passed with group options", async () => {
    const { getByRole, getAllByRole } = render(
      <DxcSelect label="test-select-label" options={groupedIconOptions} optional />
    );
    const select = getByRole("combobox");
    await userEvent.click(select);
    expect(getByRole("list").querySelectorAll("img").length).toBe(4);
