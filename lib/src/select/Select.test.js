...
describe("Select component tests", () => {
  ...
  test("Single selection: Renders with correct default value", async () => {
    const { getByText, getByRole, getAllByRole, queryByRole, container, user } = setup(
    ...
    await user.click(getAllByRole("option")[7]);
  ...
  test("Disabled select - Clear all options action must be shown but not clickable", async () => {
    const { getByRole, getByText, queryByRole, user } = setup(
  ...
  test("Non-Grouped Options - Opens listbox and renders correctly or closes it with a click on select", async () => {
    const { getByText, getByRole, getAllByRole, queryByRole, user } = setup(
  ...
  test("Non-Grouped Options - If an empty list of options is passed, the select is rendered but doestn't open the listbox", async () => {
    const { getByRole, queryByRole, user } = setup(<DxcSelect label="test-select-label" options={[]} />);
    const select = getByRole("combobox");
  ...
  test("Non-Grouped Options - Optional renders an empty first option (selected by default) with the placeholder as its label", async () => {
    const onChange = jest.fn();
    const { getByRole, getAllByRole, getAllByText, user } = setup(
      <DxcSelect
        label="test-select-label"
  ...
  test("Non-Grouped Options - Filtering options never affects the optional item until there are no coincidence", async () => {
    const { getByRole, getAllByRole, getByText, queryByText, container, user } = setup(
      <DxcSelect
        label="test-select-label"
  ...
  test("Non-Grouped Options - Renders SVG icons correctly when passed with options", async () => {
    const { getByRole, getAllByRole, user } = setup(<DxcSelect label="test-select-label" options={svg_icon_options} />);
    const select = getByRole("combobox");
  ...
  test("Non-Grouped Options - Renders string url icons correctly when passed with options", async () => {
    const { getByRole, getAllByRole, user } = setup(
  ...
  test("Non-Grouped Options: Searchable - Displays an input for filtering the list of options", async () => {
    const onChange = jest.fn();
    const { container, getByText, getByRole, getAllByRole, queryByRole, user } = setup(
    ...
    await user.click(select);
    expect(getByRole("listbox")).toBeTruthy();
    await user.type(searchInput, "08");
  ...
  test("Non-Grouped Options: Searchable - Displays 'No matches found' when there are no filtering results", async () => {
    const onChange = jest.fn();
    const { container, getByText, getByRole, user } = setup(
      <DxcSelect label="test-select-label" options={single_options} onChange={onChange} searchable />
    );
    const select = getByRole("combobox");
    const searchInput = container.querySelectorAll("input")[1];
  ...
  test("Non-Grouped Options: Searchable - Clicking the select, when the list is open, clears the search value", async () => {
    const onChange = jest.fn();
    const { container, getByText, getByRole, getAllByRole, user } = setup(
      <DxcSelect label="test-select-label" options={single_options} onChange={onChange} searchable />
    );
    const select = getByRole("combobox");
    const searchInput = container.querySelectorAll("input")[1];
  ...
  test("Non-Grouped Options: Searchable - Key Esc cleans the search value and closes the options", async () => {
    ...
    await user.type(searchInput, "Option 02");
  ...
  test("Non-Grouped Options: Searchable - While user types, a clear action is displayed for cleaning the search value", async () => {
    const onChange = jest.fn();
    const { container, getByRole, getAllByRole, queryByTitle, user } = setup(
      <DxcSelect label="test-select-label" options={single_options} onChange={onChange} searchable />
    );
    const searchInput = container.querySelectorAll("input")[1];
  ...
  test("Non-Grouped Options: Multiple selection - Displays a checkbox per option and enables the multi-selection", async () => {
    const onChange = jest.fn();
    const { getByText, getAllByText, getByRole, getAllByRole, queryByRole, container, user } = setup(
    ...
    await user.click(getAllByRole("option")[10]);
  ...
  test("Non-Grouped Options: Multiple selection - Clear action and selection indicator", async () => {
    const onChange = jest.fn();
    const { getByText, queryByText, getByRole, getAllByRole, queryByRole, getByTitle, queryByTitle, user } = setup(
      <DxcSelect label="test-select-label" options={single_options} onChange={onChange} multiple />
    );
    const select = getByRole("combobox");

    await user.click(select);
    await user.click(getAllByRole("option")[5]);
    await user.click(getAllByRole("option")[8]);
    await user.click(getAllByRole("option")[13]);
    ...
    await user.click(getByTitle("Clear selection"));
    expect(onChange).toHaveBeenCalledWith({ value: [], error: "This field is required. Please, enter a value." });
    expect(queryByRole("listbox")).toBeTruthy();
  ...
  test("Non-Grouped Options: Multiple selection - Optional option should not be added when the select is marked as multiple", async () => {
    const onChange = jest.fn();
    const { getByText, getAllByText, getByRole, getAllByRole, user } = setup(
      <DxcSelect
        label="test-select-label"
        placeholder="Choose an option"
        options={single_options}
  ...
  test("Non-Grouped Options - If an options was previously selected when its opened (by key press), the visual focus appears always in the selected option", async () => {
    ...
    await user.click(select);
    await user.click(getAllByRole("option")[4]);
  ...
  test("Non-Grouped Options - If an options was previously selected when its opened (by click and key press), the visual focus appears always in the selected option", async () => {
    ...
    await user.click(select);
    await user.click(getAllByRole("option")[15]);
    expect(queryByRole("listbox")).toBeFalsy();
  ...
  test("Grouped Options - If an empty list of options in a group is passed, the select is rendered but doestn't open the listbox", async () => {
  ...
  test("Grouped Options - Filtering options never affects the optional item until there are no coincidence", async () => {
    ...
    expect(getByText("Placeholder example")).toBeTruthy();
    expect(getAllByRole("option").length).toBe(6);
    await user.type(searchInput, "roro");
    expect(queryByText("Placeholder example")).toBeFalsy();
    expect(getByText("No matches found")).toBeTruthy();
  });
  test("Grouped Options - Renders icons correctly when passed with group options", async () => {
    const { getByRole, getAllByRole, user } = setup(
  ...
  test("Grouped Options: Searchable - Displays 'No matches found' when there are no filtering results", async () => {
    ...
    await user.click(select);
    expect(getByRole("listbox")).toBeTruthy();
    await user.type(searchInput, "very long string");
    expect(getByText("No matches found")).toBeTruthy();
  });
  ...
  test("Grouped Options: Multiple selection - Clear action and selection indicator", async () => {
    ...
    await user.click(getAllByRole("option")[8]);
    await user.click(getAllByRole("option")[13]);
    await user.click(getAllByRole("option")[17]);
  ...
  test("Multiple selection and optional - Clear action cleans every selected option but does not display an error", async () => {
    const onChange = jest.fn();
    const { getByRole, getAllByRole, getByTitle, user } = setup(
