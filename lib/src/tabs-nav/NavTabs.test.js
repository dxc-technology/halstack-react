...
describe("Tabs component tests", () => {
  ...
  test("Tabs render custom link with correct label, badge and icon", () => {
    const { getByText, getByRole } = render(
      <DxcNavTabs>
        <DxcNavTabs.Tab href="/test1" active notificationNumber={20} icon="/testIcon.png">
          <a href="/mytest" rel="author">
            Tab 1
          </a>
        </DxcNavTabs.Tab>
      </DxcNavTabs>
    ...
    expect(getByRole("tab").getAttribute("tabindex")).toBe("0");

    expect(getByRole("link").getAttribute("href")).toBe("/mytest");
    expect(getByRole("link").getAttribute("rel")).toBe("author");
  });
});
